...
batches = [
    ...
    with selected_deployments as (
        ...
        from deployments d
        left join mm_tags_deployments mm on mm.deployments_deployment_id = d.deployment_id
        left join tags t on t.tag_id = mm.tags_tag_id
        left join nodes n on d.node_id = n.node_id
        where t.name = 'FS3'
            and n.type = 'Pollinator Cam'
            and d.deployment_id not in (1198, 1203, 1215, 1224, 2150)
    )
    -- select count(*), pg_size_pretty(sum(f.file_size)) from files_image f
